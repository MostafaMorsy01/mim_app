...
class UpdateProductMobileView extends StatefulWidget {
  final SpecificationCoreModel? specCore;
  final CategoriesCoreModel? catCore;
  final product_id;

  const UpdateProductMobileView(
      {Key? key, required this.specCore, required this.catCore,required this.product_id})
      : super(key: key);

  @override
  State<UpdateProductMobileView> createState() => _UpdateProductMobileViewState();
}

class _UpdateProductMobileViewState extends State<UpdateProductMobileView> {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        flexibleSpace: TitleAppBar(
          title: getTranslated(context, "update_product"),
        ),
        elevation: 0,
      ),
      body: SizedBox(
                                        ...
                                        child: MainButton(
                                            onPressed: () async {
                                              if (_formKey.currentState!.validate()) {
                                                bool result = await addProductViewModel.spUpdateProductStore(
                                                  widget.product_id,
                                                ...
                                                if (result) {
                                                  ...
                                                  toastAppErr(
                                                      "Product does not Update to Store",
                                                      contest: context);
                                                }
                                            ...
                                            text: getTranslated(context, "update_product"),
                                            width: deviceSize.width * 0.9),
                                      ),
                                    ],
                                  )),

...
class SpAddProductWebServices {
  String? _token;

  Future<APIResponse> spAddProductStore(
    ...
    try {
      ...
      var request = http.MultipartRequest(
          'POST', Uri.parse(EndPoints.baseUrl + EndPoints.store_add_products));
...
//       }
      if (imageList.isNotEmpty) {
        for (int i = 0; i < imageList.length; i++) {
          request.files.add(http.MultipartFile.fromBytes(
              'images[$i]', File(imageList[i]).readAsBytesSync(),
  ...
  Future<APIResponse> spUpdateProductStore(
      Map<String, String> body, List<String> imageList) async {
    await SecureStorageService.readByKey("accessToken").then((value) => {
      _token = value,
    });
    ...
    try {
      ...
      var request = http.MultipartRequest(
          'POST', Uri.parse(EndPoints.baseUrl + EndPoints.update_product));
      request.headers.addAll(headers);
      // List<MultipartFile> newList = List<MultipartFile>();

...
import '../Services/shipping_method_web_service.dart';



class ShippingMethodViewModel with ChangeNotifier {
  User? user;
  String? accessToken;
  ...
  ShippingMethodCoreModel? shippingCore;
  ...
  ShippingMethodWebService webServices = ShippingMethodWebService();

  Future<void> shippingMethodData(
    ...
    notifyListeners();
    print("start");
    APIResponse? response = await webServices.fetchShippingMethod();
    print("message");
    ...
    if (response?.status == 200) {
      shippingCore = ShippingMethodCoreModel.fromJson(response?.data);
      print("dataa");
      print("done");
  ...
  Future<bool> addShipping(
      List<ShippingCompany> shippingMethod,
      BuildContext context,
      ) async {
    ...
    try {
      Map<String, dynamic> body = {
        for (int i = 0; i < shippingMethod.length; i++)
          "shippings_id[$i]": shippingMethod[i].id.toString(),

      };
      print(body);
      APIResponse response = await webServices.addShipping(body);
